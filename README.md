# ربات تلگرام مدیریت پیشرفته پست کانال با Cloudflare Workers (فارسی / English)
...
3.  **پیکربندی ورکر:**
    ...
    * **اگر از Wrangler استفاده می‌کنید:**
        * فایل `wrangler.toml` را مشابه نمونه زیر ایجاد یا ویرایش کنید:
            ```toml
            name = "your-worker-name" # نام ورکر شما
...
5.  **تنظیم Webhook تلگرام:**
    * URL ورکر خود را (مثلاً `https://your-worker-name.your-subdomain.workers.dev`) بردارید.
    * URL زیر را با مقادیر خودتان جایگزین کرده و در مرورگر اجرا کنید:
        `https://api.telegram.org/bot<YOUR_BOT_TOKEN>/setWebhook?url=<YOUR_WORKER_URL>`
    * یا از مسیر `/setwebhook` که در کد تعبیه شده استفاده کنید: `https://your-worker-name.your-subdomain.workers.dev/setwebhook` (یک بار در مرورگر اجرا کنید).
...
3.  **Configure the Worker:**
    ...
    * **If using Wrangler:**
        * Create or edit the `wrangler.toml` file similar to the example below:
            ```toml
            name = "your-worker-name" # Your Worker's name
...
5.  **Set Telegram Webhook:**
    * Get your Worker's URL (e.g., `https://your-worker-name.your-subdomain.workers.dev`).
    * Replace the placeholders in the URL below and open it in a web browser:
        `https://api.telegram.org/bot<YOUR_BOT_TOKEN>/setWebhook?url=<YOUR_WORKER_URL>`
    * Alternatively, use the `/setwebhook` path defined in the code: `https://your-worker-name.your-subdomain.workers.dev/setwebhook` (run once in a browser).
